...
module.exports = class AhegaoCommand extends Command {
    constructor(client) {
        super(client, {
            ...
            memberName: 'solo',
            group: 'nsfw',
            description: 'Sends a random Solo Image from Danbooru',
            guarded: true,
            guildOnly: true,
        })
    }
    run(msg) {
        if (msg.channel.nsfw === true) {
            booru.posts({
                ...
                msg.react("đ");
                msg.delete(1500);
                msg.log(`Someone used Solo Command ${guild.name} `);
                const embed = new Discord.RichEmbed()
                    .setDescription("Here are some hot singles")
                    .setColor('#FFFFFF')
                    .setImage(url)
